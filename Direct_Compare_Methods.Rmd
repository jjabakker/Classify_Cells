...
# Global parameters still used:

# top_genes
# features_limit
  
train_and_test <- function (dataset_name, data, labels, method, preproc_method) {
...
for (i in 1 : dim(medianF1_table)[1]) {
  for (j in 1 : dim(medianF1_table)[2]) {
    report_row = data.frame(Method    = rownames(medianF1_table)[i],
                            ModelName = dataset_name,
                            PreProc   = colnames(medianF1_table)[j],
                            MedianF1  = medianF1_table[i, j],
                            duration  = duration_table[i, j])
    report_out <- rbind(report_out, report_row)
  }
}

save_report_data (report_out         = report_out,
                  path               = rdata_path,
                  model_dataset_name = dataset_name)
