...
source("Code/Train/seurat_cluster.R")
...
calc_coherence <- function(seurat_object) {
  ...
  for (i in 0:(nr_of_clusters - 1)) {
    ...
    cluster$Count      <- sc
    cluster            <- select(cluster, "Coherence", "Count", everything())
...
Repeat this chunk until it looks you have the required number of clusters
...
cluster_resolution   <- find_best_resolution(seurat_object) 
seurat_object        <- FindClusters(seurat_object, resolution = cluster_resolution)
