...
for (row in 1:nrow(to_review)) {
  ...
  genes_in_model             <- colnames(data_of_model)
  ...
  genes_intersect           <- intersect(genes_in_data, genes_in_model)
  data_of_model             <- data_of_model[ , genes_intersect]
  ...
  ldata_of_model            <- as.data.frame(data_of_model[ , top_genes])
...
report_out1$Genes <- 1000
report_out1$Mode  <- "Anticipate 2"
report            <- report_out1
...
report_anticipate_2 = report
save(report_anticipate_2, file = file.path(rdata_path, "report_anticipate_2.Rdata"))
