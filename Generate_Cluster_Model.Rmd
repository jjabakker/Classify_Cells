...
coherence_table      <- ret$coherence_table
...
cluster_table        <- ret$cluster_table
weighted_coherence   <- ret$weighted_coherence
...
if (method == "svmLinear") {
  ...
  save_cluster_model(path                = rdata_path, 
                     ...
                     top_genes           = top_genes,
                     features_limit      = length(top_genes),
...
if (method == "gbm") {
  ...
  predicted_classes <- predict(model, pp_test_data[,-1])
  cm                <- confusionMatrix(pp_test_data[,1],
                                       predicted_classes,
                                       mode = "everything",
                                       dnn  =c("Reference", "Predicted"))
